...
class Bank:
    ...
    def convertMoney(self, money: "Money", to_currency: Currency) -> "Money":
        ...
        if self.is_same_currency(from_currency, to_currency):
            return money
    ...
    def get_rate(self, from_currency: Currency, to_currency: Currency) -> float:
        ...
        if self.hasExchangeRate(from_currency, to_currency):
            return self._exchange_rate[f"{from_currency.value}->{to_currency.value}"]
    ...
    def is_same_currency(self, from_currency: Currency, to_currency: Currency) -> bool:
